...
export async function GET(req: Request) {
  ...
  const now = new Date();
  const jobs = await db.scheduledPost.findMany({
    where: { status: 'scheduled', scheduledAt: { lte: now }, content: { status: 'scheduled' } },
    include: { content: true, instagramAccount: true },
    take: 10,
  });
  ...
  for (const job of jobs) {
    if (job.instagramAccount.accessTokenEncrypted && job.instagramAccount.tokenExpiresAt && job.instagramAccount.tokenExpiresAt.getTime() - Date.now() < 7 * 24 * 60 * 60 * 1000) {
      ...
      } catch { /* publishing will surface an invalid-token error */ }
    }
    ...
    if (!job.instagramAccount.accessTokenEncrypted || !job.content.mediaUrls[0]) {
      ...
      await db.content.update({ where: { id: job.contentId }, data: { status: 'failed' } });
    ...
    try {
      ...
      await db.content.update({ where: { id: job.contentId }, data: { status: 'published', publishedAt: new Date(), externalPostId: result.id } });
      await db.auditLog.create({ data: { userId: job.content.userId, action: 'POST_PUBLISHED', status: 'success', details: { scheduledPostId: job.id, externalPostId: result.id } } });
